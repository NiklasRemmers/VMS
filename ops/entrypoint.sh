...
from vms.infra.kms import load_master_key, save_secrets, clear_cache
clear_cache()
...
import os
os.environ['KMS_MASTER_KEY_PATH'] = '$KMS_KEY'
from vms.domain.database import init_db
init_db()
print('✓ Tables ready')
"
...
engine = get_engine()
with engine.connect() as conn:
    for col, coltype in [('return_note', 'TEXT'), ('returned_at', 'TIMESTAMPTZ')]:
...
exec gunicorn --config gunicorn.conf.py vms.app:app
