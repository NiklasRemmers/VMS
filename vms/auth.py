...
from vms.domain.database import get_session, init_db as db_init
from vms.domain.models import User as UserModel
# Auf Modulebene, nicht im try-Block: der Name muss beim Auswerten der
# except-Klausel garantiert existieren, auch wenn eine frühere Zeile im try wirft.
from vms.infra.security import DecryptionError

# Modul-Logger statt current_app.logger: die User-Methoden unten laufen auch
# ohne App-Kontext (Setup-Skripte, Test-Fixtures), wo current_app werfen würde.
...
class User(UserMixin):
    ...
    def create(username, password, display_name=None, email=None, is_active=True):
        ...
        try:
            ...
            # Ohne das Log sind doppelter Benutzername, verletzte Constraint und
            # DB-Ausfall für den Aufrufer ununterscheidbar -- er sieht nur None.
            logger.exception("User.create fehlgeschlagen für username=%r email=%r",
                             username, email)
            return None
    ...
    def create_invitation(email):
        ...
        try:
            ...
            logger.exception("Einladung konnte nicht angelegt werden für email=%r", email)
            return None
    ...
    def complete_invitation(self, username, password, display_name):
        ...
        try:
            ...
            logger.exception("Einladung konnte nicht abgeschlossen werden für user_id=%r "
                             "username=%r", self.id, username)
            return False
...
def send_invitation_email(email_addr, invite_url):
    ...
    if current_user.is_authenticated:
        try:
            ...
            # Gespeichertes SMTP-Passwort ist nicht lesbar (meist: Fernet-Key hat
            # zwischen KMS- und SECRET_KEY-Zweig gewechselt, s. security.py).
            # Bewusster Fallback auf globales Flask-Mail — aber laut protokolliert,
            # damit die Ursache nicht als "kein Passwort gesetzt" untergeht.
            current_app.logger.error(
                f"SMTP-Passwort von Benutzer {current_user.id} nicht entschlüsselbar, "
                f"nutze globalen Mail-Fallback: {e}"
            )
        except Exception as e:
            current_app.logger.error(f"Error fetching SMTP settings from DB: {e}")
