...
# secrets.enc lives at the repo root (this module sits at vms/infra/); overridable via env.
DEFAULT_SECRETS_PATH = os.environ.get(
    'KMS_SECRETS_PATH',
    os.path.join(
...
def load_master_key(path: str = None) -> bytes:
    ...
    # Validate before caching: an invalid key must never end up in the global,
    # or the next call would hit the cache branch and return it without raising.
    if len(key) < 32:
...
def save_secrets(secrets_dict: dict, path: str = None, master_key: bytes = None):
    ...
    # Cache auf den geschriebenen Stand ziehen: sonst liefert ein load_secrets()
    # im selben Prozess weiter den alten Inhalt aus _secrets.
    global _secrets
    _secrets = dict(secrets_dict)
...
def get_secret(key: str, default: str = None) -> str:
    ...
    # Nur "KMS nicht eingerichtet" ist ein legitimer Grund für den Default.
    # Entschlüsselungs- und Parse-Fehler sind Konfigurationsfehler und müssen
    # propagieren — sonst läuft die App still mit leeren Secrets weiter.
    try:
