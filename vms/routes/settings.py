...
# Altlast: wird benutzt, wenn beim Speichern keine Projekt-ID angegeben wird.
# Die Zahl stammt aus der ursprünglichen Einzelnutzer-Installation und ist für
# jede andere Instanz ein fremdes Projekt -- siehe Notiz im Blocker-Abschnitt.
DEFAULT_KANBOARD_PROJECT_ID = 25
...
def update_email_manual():
    ...
    # Nichts schreiben, wenn die Verschlüsselung scheitert — sonst meldet die Route
    # Erfolg, während das Passwort leer bleibt bzw. das alte stehen bleibt.
    try:
        ...
        current_app.logger.error(f"E-Mail-Einstellungen nicht gespeichert: {e}")
        return jsonify({'error': 'Passwort konnte nicht verschlüsselt werden. '
                                 'Einstellungen wurden nicht gespeichert.'}), 500
...
def update_kanboard():
    ...
    if project_id in (None, ''):
        ...
        try:
            ...
            # Vorher wurde eine unlesbare Eingabe still durch den Default ersetzt --
            # der Nutzer landete dann in einem fremden Kanboard-Projekt, ohne dass
            # die Oberfläche etwas anderes anzeigte als "gespeichert".
            return jsonify({'error': 'Projekt-ID muss eine Zahl sein'}), 400
        if project_id <= 0:
            return jsonify({'error': 'Projekt-ID muss größer als 0 sein'}), 400
    ...
    try:
        ...
        current_app.logger.error(f"Kanboard-Einstellungen nicht gespeichert: {e}")
        return jsonify({'error': 'Token konnte nicht verschlüsselt werden. '
                                 'Einstellungen wurden nicht gespeichert.'}), 500
...
def upload_signature():
    ...
    if file and file.filename.endswith('.png'):
        try:
            ...
            current_app.logger.exception("Fehler beim Speichern der Unterschrift")
            flash('Fehler beim Speichern der Unterschrift', 'error')
    else:
    ...
    with get_session() as s:
        ...
        if settings and settings.encrypted_signature:
            try:
                ...
                current_app.logger.exception("Signatur konnte nicht geladen werden")
                return jsonify({'error': 'Signatur konnte nicht geladen werden'}), 500
