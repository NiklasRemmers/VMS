...
from vms.domain.models import StorageLocation, CodeShareLink, format_de_date, ACTIVE_STATUSES
from vms.auth import login_required, current_user, limiter
...
def send_codes():
    ...
    from vms.clients.email_client import get_candidate_by_id
    from vms.auth import send_plain_email
    ...
    url = url_for('verleih.public_codes', token=token, _external=True)
...
def get_assignable_loans():
    ...
    } for c in candidates if c.get('status') in ACTIVE_STATUSES]
    return jsonify(loans)


# Aus app.py hierher gezogen: das Token wird in diesem Modul erzeugt
# (_get_or_create_share_token), also gehört die einlösende Route daneben.
@verleih_bp.route('/codes/<token>')
...
def public_codes(token):
    ...
    from vms.domain.database import get_session
    from vms.domain.models import CodeShareLink, StorageLocation, format_de_date
    from vms.clients.kanboard_client import _parse_date, _today
